...
/**
 * Process an array of files concurrently, up to `concurrency` at a time.
 * Returns merged stats.
 */
async function processWithConcurrency(files, processFn, concurrency = 16) {
    const mergedStats = { files: 0, total: 0, processed: 0, skipped: 0, errors: 0, reasons: {} };
    let index = 0;

    async function worker() {
        while (index < files.length) {
            const i = index++;
            const stats = await processFn(i);
            mergedStats.files++;
            mergedStats.total = (mergedStats.total || 0) + (stats.total || 0);
            mergedStats.processed += stats.processed;
            mergedStats.skipped += stats.skipped;
            mergedStats.errors += stats.errors;
            Object.keys(stats.reasons).forEach(reason => {
                mergedStats.reasons[reason] = (mergedStats.reasons[reason] || 0) + stats.reasons[reason];
            });
        }
    }
...
async function main() {
    ...
    // Process in order: problems → steps → hints (so all use SRE; problems/steps finish first)
    ...
    if (options.types.includes('problems')) {
        ...
        if (problemFiles.length > 0) {
            ...
            const merged = await processWithConcurrency(problemFiles, async (i) => {
                const file = problemFiles[i];
                if (options.verbose || i % 50 === 0) {
                    console.log(`[${i + 1}/${problemFiles.length}] ${path.relative(process.cwd(), file)}`);
                }
                return processProblemFile(file, options, stateMap);
            });
            totalStats.problems.files = merged.files;
            totalStats.problems.processed = merged.processed;
            totalStats.problems.skipped = merged.skipped;
            totalStats.problems.errors = merged.errors;
            totalStats.problems.reasons = merged.reasons;
        }
    }
    ...
    if (options.types.includes('steps')) {
        ...
        if (stepFiles.length > 0) {
            ...
            const merged = await processWithConcurrency(stepFiles, async (i) => {
                const file = stepFiles[i];
                if (options.verbose || i % 50 === 0) {
                    console.log(`[${i + 1}/${stepFiles.length}] ${path.relative(process.cwd(), file)}`);
                }
                return processStepFile(file, options, stateMap);
            });
            totalStats.steps.files = merged.files;
            totalStats.steps.processed = merged.processed;
            totalStats.steps.skipped = merged.skipped;
            totalStats.steps.errors = merged.errors;
            totalStats.steps.reasons = merged.reasons;
        }
    }
    ...
    if (options.types.includes('hints')) {
        ...
        if (hintFiles.length > 0) {
            ...
            const merged = await processWithConcurrency(hintFiles, async (i) => {
                const file = hintFiles[i];
                if (options.verbose || i % 50 === 0) {
                    console.log(`[${i + 1}/${hintFiles.length}] ${path.relative(process.cwd(), file)}`);
                }
                return processPathwayFile(file, options, stateMap);
            });
            totalStats.hints.files = merged.files;
            totalStats.hints.total = merged.total;
            totalStats.hints.processed = merged.processed;
            totalStats.hints.skipped = merged.skipped;
            totalStats.hints.errors = merged.errors;
            totalStats.hints.reasons = merged.reasons;
        }
    }
    ...
    if (!options.dryRun) {
        ...
        const anyUpdated = totalStats.problems.processed > 0 || totalStats.steps.processed > 0 || totalStats.hints.processed > 0;
        if (anyUpdated) {
            console.log('\nUpdating generated content pool (preprocess)...');
            try {
                const preprocessPath = path.join(__dirname, '../../tools/preprocessProblemPool.js');
                execSync(`node "${preprocessPath}"`, { stdio: 'inherit', cwd: path.resolve(__dirname, '../../..') });
            } catch (err) {
                console.warn('Preprocess failed (run manually: node src/tools/preprocessProblemPool.js):', err.message);
            }
        }
    }
