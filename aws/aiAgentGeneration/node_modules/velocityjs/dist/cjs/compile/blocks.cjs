...
class BlockCommand extends baseCompile.Compile {
  getBlock(block) {
    const ast = block[0];
    let ret = "";
    switch (ast.type) {
  ...
  getMacro(ast, bodyContent) {
    ...
    const contextId = `macro:${ast.id}:${utils.guid()}`;
    (args || []).forEach((ref, i) => {
      if (!("id" in ref)) {
        return;
      }
  ...
  eval(str, local, contextId) {
    ...
    const asts = Array.isArray(str) ? str : parse.parse(str);
